...
select @accountingId = 2, @month = 05, @year = 2014
...
WHERE w.AccPeriod IS NULL and WaybillState >1 and ReturnDate < @d
...
INNER JOIN Vehicle v ON v.VehicleId = w.VehicleId
inner join TransportColumn tc on tc.ColumnId = ISNULL(v.ColumnId,5) and tc.AccountingId = @accountingId
WHERE year(RefuellingDate)=@year AND MONTH(refuellingDate)=@month
